...
const AcceptQuotePage = () => {
  ...
  const handleAcceptQuote = async () => {
    ...
    try {
      ...
      const clientData = {
        ...
        last_name:
          quoteDetails.customerName.split(" ").slice(1).join(" ") || "N/A",
      ...
      const [client, times] = await Promise.all([
        createJobberClient(clientData),
        getAvailableTimes({startDate}),
      ])
      setJobberClient(client)
      console.log("Jobber client created:", client)

      // Step 2: Apply available time slots
  ...
  if (!quoteDetails) {
    return (
      <Layout headTitle="Loading Quote...">
        <div className="container_page">Loading quote details...</div>
      </Layout>
    )
  }
  ...
  return (
    <Layout headTitle={`Accept Quote: ${quoteDetails.serviceDescription}`}>
      ...
      >
        ...
        <p>
          <strong>Amount:</strong> ${quoteDetails.amount.toFixed(2)}
        </p>
        <p>
        ...
        {currentStep === 1 && (
          <div className="step-accept-quote">
            ...
            <p>
              Please review your quote details above. Click below to accept and
            ...
            {selectedTime && (
              <div style={{marginTop: "20px"}}>
                ...
                >
                  {isLoading
                    ? "Processing..."
                    : "Confirm Time & Proceed to Payment"}
                </button>
              </div>
            )}
          </div>
